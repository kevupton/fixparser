...
describe('FIXParser', () => {
    ...
    describe('#parse: unknown SOH "/"', () => {
        it('should parse the message, SOH "/"', () => {
            const fix: string =
                '8=FIX.4.2/9=106/35=A/34=1/49=U1par/50=U1fix/52=20090206-21:13:36.887/56=FixServer/98=0/108=30/141=Y/553=U1fix/554=hotspot/10=061/';
            expect(fixParser.parse(fix)[0].data.length).toEqual(14);
            expect(fixParser.parse(fix)[0].messageString).toMatchSnapshot();
        });
    });
    ...
    describe('#parse: special characters', () => {
        it('should parse the message', () => {
            const fix: string =
                '8=FIX.4.2|9=51|35=0|34=703|49=A!B@C#D$E%F^G&H*I(J)_K-+L\\M.N`O~P Q\\\\R\\\\\\S/T//U///V{W{{X{{{Y]Z|52=20100130-10:53:40.830|56=XYZ|10=249|';
            const message: Message = fixParser.parse(fix)[0];
            expect(message.getField(Fields.SenderCompID)!.value).toBe(
                'A!B@C#D$E%F^G&H*I(J)_K-+L\\M.N`O~P Q\\\\R\\\\\\S/T//U///V{W{{X{{{Y]Z',
            );
        });
    });

    describe('#parse: tag number and value in text', () => {
        it('should parse the message', () => {
            const fix: string =
                '8=FIX.4.2|9=51|35=0|34=703|49=ABC|58=The value of 43=5 is a test|52=20100130-10:53:40.830|56=XYZ|10=249|';
            const message: Message = fixParser.parse(fix)[0];
            expect(message.getField(Fields.Text)!.value).toBe('The value of 43=5 is a test');
        });
    });

    describe('#parse: invalid message', () => {
        it('should return null when having incorrect message', () => {
            const fix: string = 'aabbccddeeffgghhii';
            expect(fixParser.parse(fix).length).toEqual(0);
            expect(fixParser.parse(fix)).toMatchSnapshot();
        });
    });

    describe('#parse: SDBK Price Correction of Previous Execution', () => {
        let messages: Message[] = fixParser.parse(
                '8=FIX.4.2^A 9=440^A 35=8^A 128=LZJ^A 34=549^A 49=CCG^A 56=LEH_LZJ02^A 52=20100302- 22:36:15^A 55=IOC^A 37=NF 0039/03022010^A 11=NF 0039/03022010^A 17=NF 0039/03022010 001001002^A 20=2^A 39=2^A 150=2^A 54=1^A 38=100^A 40=1^A 59=0^A 31=49.3700^A 32=100^A 14=0^A 6=0^A 151=0^A 60=20100302-22:36:16^A 58=Trade correction^A 19=NF 0039/03022010 001001001^A 1=ABC123ZYX^A 30=N^A 207=N^A 47=A^A 9430=NX^A 9483=000010^A 9578=1^A 9425=5^A 9579=0000100002^A 9704=0000100001^A 382=1^A 375=TOD^A 337=0000^A 437=100^A 438=1736^A 29=1^A 63=0^A 9440=001001002^A 10=235^A ',
            ),
            index = 0;
        ...
        it('should have LastPx value 49.3700', () => {
            ...
            expect(messages[0].data[index].value).toBe(49.37);
            index++;
        });
        ...
        it('should get the correct field', () => {
            expect(messages[0].getField(Fields.ContraTradeTime)!.tag).toBe(438);
            expect(messages[0].getField(Fields.ContraTradeTime)!.value).toBe('1736');
        });
    });

    describe('#parse: New order', () => {
        let messages: Message[] = fixParser.parse(
                '8=FIX.4.2|9=154|35=E|49=INST|56=BROK|52=20050908-15:51:22|34=200|66=14|394=1|68=2|73=2|11=order- 1|67=1|55=IBM|54=2|38=2000|40=1|11=order-2|67=2|55=AOL|54=2|38=1000|40=1|',
            ),
            index = 0;
        expect(messages).toMatchSnapshot();
    ...
    describe('#parse: ExecutionReport', () => {
        let messages: Message[] = fixParser.parse('8=FIX.4.4\x019=1753\x0135=8\x01523=S|E|B Auto-pricing\x01'),
            index = 0;
        expect(messages).toMatchSnapshot();
    ...
    describe('#getBriefDescription', () => {
        describe('should generate a brief description', () => {
            ...
            it('CBS Direct-To-Handheld New Order Single', () => {
                messages = fixParser.parse(
                    '8=FIX.4.2\x019=154\x0135=D\x0134=57\x0149=ABC_DEFG04\x0152=20090402-18:15:40\x0156=CCG\x01115=XYZ\x01143=100\x0111=NF 0646/04022009\x0154=1\x0138=100\x0155=PAC\x0140=1\x0159=0\x0147=A\x0160=20090402-18:15:40\x0121=1\x01207=N\x0110=013\x01',
                );
            ...
            it('Cancel Replace (changing quantity) of previously reduced order', () => {
                messages = fixParser.parse(
                    '8=FIX.4.2\x019=197\x0135=G\x0134=131\x0149=ABC_DEFG01\x0152=20090325-15:20:32\x0156=CCG\x01115=XYZ\x0111=NF 0578/03252009\x0137=NF 0576/03252009\x0141=NF 0576/03252009\x0154=1\x0138=2000\x0155=CVS\x0140=2\x0144=25.47\x0159=0\x0147=A\x0160=20090325-15:20:32\x0121=1\x01207=N\x0110=172\x01',
                );
            ...
            it('Closing Offset (New Order Single)', () => {
                messages = fixParser.parse(
                    '8=FIX.4.2\x019=156\x0135=D\x0134=124\x0149=ABC_DEFG04\x0152=20100208-18:51:42\x0156=CCG\x01115=XYZ\x0111=NF 0015/02082010\x0154=2\x0138=1000\x0155=RRC\x0140=2\x0144=55.36\x0159=0\x011=ABC123ZYX\x0121=1\x01207=N\x0147=A\x019487=CO\x0110=050\x01',
                );
            ...
            it('MatchPoint (New Order Single)', () => {
                messages = fixParser.parse(
                    '8=FIX.4.2\x019=153\x0135=D\x0134=11\x0149=ABC_DEFG04\x0152=20091216-17:23:52\x0156=CCG\x0157=ARCA\x01115=XYZ\x01128=MP\x0111=NF 0921/12162009\x0154=2\x0138=1000\x0155=MMM\x0140=1\x0159=0\x011=ABC123ZYX\x0121=1\x01207=N\x0147=A\x0110=172\x01',
                );
            ...
            it('Sub-Penny (New Order Single)', () => {
                messages = fixParser.parse(
                    '8=FIX.4.2\x019=147\x0135=D\x0134=215\x0149=ABC_DEFG04\x0152=20100208-19:36:45\x0156=CCG\x01115=XYZ\x0111=NF 0016/02082010\x0154=1\x0138=1000\x0155=VOD\x0140=2\x0144=.995\x0159=0\x011=ABC123ZYX\x0121=1\x01207=N\x0147=A\x0110=100\x01',
                );
            ...
            it('Dot Reserve Order Type (Max Floor)', () => {
                messages = fixParser.parse(
                    '8=FIX.4.2\x019=154\x0135=D\x0134=578\x0149=LEH_LZJ02\x0152=20100302-22:50:34\x0156=CCG\x01115=LZJ\x0111=NF 0040/03022010\x0154=1\x0138=1000\x0155=IOC\x0140=2\x0144=49.38\x0159=0\x011=ABC123ZYX\x0121=1\x01207=N\x0147=A\x01111=0\x0110=121\x01',
                );
            ...
            it('BBSS Max Quantity Order (99,999,900 shares)', () => {
                messages = fixParser.parse(
                    '8=FIX.4.2\x019=162\x0135=D\x0134=3\x0149=LEH_LZJ02\x0152=20100303-15:17:05\x0156=CCG\x0157=4\x01115=LZJ\x01143=ER\x0111=NF 0041/03032010\x0154=1\x0138=99999900\x0155=MMM\x0140=2\x0144=50.00\x0159=0\x011=ABC123ZYX\x0121=1\x01207=N\x0147=A\x0110=069\x01',
                );
            ...
            it('Symbol with a Suffix', () => {
                messages = fixParser.parse(
                    '8=FIX.4.2\x019=150\x0135=D\x0134=13\x0149=LEH_LZJ02\x0152=20100303-15:21:52\x0156=CCG\x01115=LZJ\x0111=NF 0043/03032010\x0154=1\x0138=100\x0155=ADM\x0165=PRA\x0140=2\x0144=25\x0159=0\x011=ABC123ZYX\x0121=1\x01207=N\x0147=A\x0110=237\x01',
                );
            ...
            it('New Order (order to be replaced)', () => {
                messages = fixParser.parse(
                    '8=FIX.4.2\x019=157\x0135=D\x0134=117\x0149=ABC_DEFG01\x0152=20090325-15:14:27\x0156=CCG\x01115=XYZ\x0111=NF 0573/03252009\x0154=1\x0138=1000\x0155=CVS\x0140=2\x0144=25.47\x0159=0\x0147=A\x0160=20090325-15:14:27\x0121=1\x01207=N\x0110=210\x01',
                );
    ...
    describe('#getField: ExecutionReport', () => {
        ...
        it('should get the correct field', () => {
            expect(messages[0].getField(Fields.MsgSeqNum)!.tag).toBe(34);
            expect(messages[0].getField(Fields.MsgSeqNum)!.value).toBe(833);
            expect(messages).toMatchSnapshot();
        });
    });
    ...
    describe('#parse: checksum validation', () => {
        const messages: Message[] = fixParser.parse(
            '8=FIX.4.4|9=109|35=AN|49=ctrader.1546785|56=cServer|57=TRADE|50=1546785|34=2|52=20190109-10:48:02|710=140511|721=5305215|10=175|',
        );
        it('should validate checksum correctly', () => {
            expect(messages[0].checksumValid).toBe(false);
            expect(messages[0].encode()).toMatchSnapshot();
        });

        const messages1: Message[] = fixParser.parse(
            '8=FIX.4.4|9=109|35=AN|49=ctrader.1546785|56=cServer|57=TRADE|50=1546785|34=2|52=20190109-10:48:02|110=555|710=140511|721=5305215|10=221|',
        );
        it('should validate checksum correctly', () => {
            expect(messages1[0].checksumValid).toBe(true);
            expect(messages1[0].encode()).toMatchSnapshot();
        });
    });

    describe('ExecutionReport (Snapshot)', () => {
        let messages: Message[] = fixParser.parse(
            '8=FIX.4.2|9=266|35=8|49=ABC|56=XYZ|50=AZ12|57=NA|34=833|52=20100130-08:00:51.992|55=GLD|48=PL11YA|167=FUT|207=LIFFE|1=AA1|37=ABC1|17=INDNTHDOG|58=Fill|200=201009|205=13|32=25|151=0|14=25|54=2|40=2|77=O|59=0|150=2|20=0|39=2|442=1|44=99.06|38=25|31=99.06|6=99.06|60=20100130-08:00:52|10=136|',
        );
